...
Binance声明`client_id_metadata_document_supported=true`，因此`client_id`不是任意名称，而是Binance服务器可以读取的公网HTTPS JSON地址。参考[模板](oauth-client-metadata.example.json)部署到项目控制的HTTPS域名后，必须把JSON里的`client_id`改成该文件自身的最终URL。不要使用Codex或其他客户端拥有的metadata URL。

Binance advertises `client_id_metadata_document_supported=true`, so `client_id` is a public HTTPS JSON URL retrievable by Binance, not an arbitrary name. Deploy the [template](oauth-client-metadata.example.json) on a project-controlled HTTPS origin and set its `client_id` to that document's final URL. Do not reuse metadata owned by Codex or another client.
