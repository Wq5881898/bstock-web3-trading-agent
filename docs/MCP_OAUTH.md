...
Binance声明`client_id_metadata_document_supported=true`，因此`client_id`不是任意名称，而是Binance服务器可以读取的公网HTTPS JSON地址。项目默认使用jsDelivr从公开GitHub仓库的不可变`oauth-client-v1`标签分发无凭据身份文件：`https://cdn.jsdelivr.net/gh/Wq5881898/bstock-web3-trading-agent@oauth-client-v1/site/oauth/bstock-web3-agent.json`；JSON中的`client_id`等于该文件自身URL。固定标签避免`@main`在不同CDN节点缓存不一致。首次使用前必须验证200/`application/json`及内容一致。GitHub Pages工作流保留为可选替代，但不是当前必备设置。不要使用Codex或其他客户端拥有的metadata URL。[通用模板](oauth-client-metadata.example.json)仅供更换域名时参考。

Binance advertises `client_id_metadata_document_supported=true`, so `client_id` is a public HTTPS JSON URL retrievable by Binance, not an arbitrary name. The project defaults to jsDelivr serving the credential-free identity file from the public repository's immutable `oauth-client-v1` tag: `https://cdn.jsdelivr.net/gh/Wq5881898/bstock-web3-trading-agent@oauth-client-v1/site/oauth/bstock-web3-agent.json`; the JSON `client_id` equals its own URL. The fixed tag prevents divergent `@main` caches across CDN nodes. HTTP 200, `application/json` and exact content must be verified before first use. The Pages workflow remains an optional alternative, not a required setting. Do not reuse metadata owned by Codex or another client. The [generic template](oauth-client-metadata.example.json) is only for a future domain change.
