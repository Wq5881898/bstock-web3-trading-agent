...
| Phase 4 | 进行中（第2步已启动）/ In progress (step 2 started) | 既有MCP账户只读核对、成交账本和经确认的会话权益基线已建立；BTCUSDT只观察进程持续运行。2026-09-23真实七项MCP读取到策略`HOLD`/持久会话`RUNNING`的单次周期已验收。2026-09-22人工确认的MCP市价SELL已成交并对账，但不是策略SELL验收；持续宿主编排、策略BUY/SELL、故障演练及完整24小时仍待监督。/ Existing MCP readback, fill ledger, and confirmed session-equity baseline are established; the BTCUSDT observer runs observe-only. One live seven-read MCP-to-strategy `HOLD` / durable `RUNNING` cycle passed on 2026-09-23. The operator-confirmed manual SELL filled and reconciled, but does not count as a strategy SELL. Continuous host orchestration, strategy BUY/SELL, failure drills, and 24-hour supervision remain. |
...
### Phase 4 当前实际缺口 / Actual Phase 4 gaps

1. 观察器仍是 `OBSERVE_ONLY`：已补快照逐轮重读和过期动作失效。新增单次周期和同进程严格导入入口，把新鲜MCP宿主回执、权益/成交账本、公开K线和持久会话接到候选文件。真实两步文件交接曾超过15秒并安全失败；2026-09-23 同进程路径在既有MCP七项只读刷新后完成真实时序验收，约14.9秒返回策略 `HOLD` / 会话 `RUNNING` / 无候选、无订单。15秒门槛未放宽。“观察信号自动唤醒Codex宿主七项读取、桌面逐笔确认、终态导入并持续下一轮”的编排仍未接线。不能把手动 MCP 卖单算成策略自动执行。
2. 已按用户“继续”采纳 dust 规则：原始 BTC 数量和成本继续完整对账；经 `LOT_SIZE`、`MARKET_LOT_SIZE` 与最小名义金额核验后，低于可交易门槛的余量单独计入 dust。策略持仓与买入仓位上限只按可交易部分计算；卖出候选可保留不可交易余量，但不能静默留下另一笔仍可交易的仓位。此规则的离线测试已通过，真实策略 BUY/SELL 尚未验收。
3. 2026-09-23 已经用现有 MCP 完成卖后七项只读刷新，严格导入本地忽略提交的回执；完整订单/成交、账户余额与零挂单核对通过，当前余量被识别为 dust。观察器已切到卖后回执，原始策略判断不再沿用卖前仓位。该回执过 15 秒即不再允许生成候选；每一笔策略动作仍须由现有 Codex MCP 宿主重新刷新证据、核对信号/持仓并取得当笔确认。不得新建账户、API Key 或替换执行通道。
4. 卖后两笔完整成交现已通过项目账本逻辑同步至本地忽略提交的 `runtime/desktop/mcp/live/btcusdt-fills.json`；原始 dust 及其成本保留。观察器现在不会把过期账户快照下的 K 线记为已处理，宿主刷新后可以对同一 K 线重新核验。真实票据已改为必须读取明确确认的权益基线，从新鲜账户回执计算会话累计亏损；无法由成交解释的资金变化失败关闭。持续宿主编排仍待接通。
5. 用户在权益基线提问后回复“继续”，据此于 2026-09-23 再次通过既有 MCP 完成七项只读刷新并严格导入：账户指纹不变、两笔完整历史成交和订单、零挂单。然后以新鲜快照在本地初始化会话权益基线，初始累计亏损为零；仅写入忽略提交的本地风险账本，没有交易、转账、重新授权或创建账户。基线建立不放宽任何订单的15秒新鲜度及逐笔确认要求。
6. 2026-09-23 宿主自动化可行性核查：本机 Codex CLI 的真实配置中已启用同一个 `binance-agent-os`，不需要也未进行重新登录；但非交互 `codex exec` 默认审批策略为 `never`。临时只放行 `tool_search` 后，工具发现成功；`spot.getAccount` 在该服务中不能作为独立工具直接调用，必须经过可转发多种操作的通用 `tool_execute`，而该调用被拒绝：`MCP tool call requires approval, but approval policy is never`。不能为了自动读取而无条件批准通用 `tool_execute`，因为它也能转发真实写操作。这是当前“持续自动唤醒宿主”缺口，不是账户或 OAuth 故障；未改全局配置、未调用写工具。

1. The observer remains `OBSERVE_ONLY`. It reloads snapshots and invalidates stale actions. One-shot cycle and same-process strict-import entry points join a fresh host receipt, equity/fill ledgers, public candles, and the durable session. A live two-step file handoff exceeded 15 seconds and failed closed. On 2026-09-23, the same-process path passed a live seven-read MCP timing check, returning strategy `HOLD` / session `RUNNING` / no candidate or order in approximately 14.9 seconds; the freshness limit was not relaxed. Continuous signal-triggered host reads, desktop per-order confirmation, terminal import, and resumption are not yet wired. The manual MCP sell is not a strategy execution.
2. The operator's “continue” adopted the dust rule: raw BTC quantity and cost remain fully reconciled; after `LOT_SIZE`, `MARKET_LOT_SIZE`, and minimum-notional checks, untradable residue is tracked separately. Strategy position and the entry position-cost cap use the tradable portion. A SELL candidate may retain only untradable residue, never silently leave another tradable lot. Offline tests pass; live strategy BUY/SELL acceptance remains outstanding.
3. On 2026-09-23 the existing MCP completed a seven-read post-sell refresh and a strict local import. Complete orders/fills, balances, and no open orders reconciled; the remaining position is classified as dust. The observer now uses this post-sell receipt rather than the pre-sell position. After 15 seconds it is no longer valid for a candidate. Every strategy action still needs a new host read, signal/position check, and per-order confirmation; no new account, API key, or transport switch is allowed.
4. Both complete post-sale fills have now been synced through the application ledger to the local ignored `runtime/desktop/mcp/live/btcusdt-fills.json`; raw dust and its cost remain recorded. The observer no longer consumes a candle while its account snapshot is stale, so a refreshed host snapshot can recheck that candle. Live tickets now require an explicitly confirmed equity baseline and compute cumulative session loss from fresh receipts, failing closed on balance changes unexplained by fills. Continuous host orchestration is still pending.
5. Following the operator's “continue” response to the baseline question, a new seven-read refresh through the existing MCP on 2026-09-23 strictly imported with the same account fingerprint, two complete historical fills/orders, and no open orders. A local session-equity baseline was then initialized from the fresh snapshot with zero initial cumulative loss. Only the ignored local risk ledger was written: no order, transfer, reauthorization, or account creation occurred. The baseline does not relax 15-second preflight freshness or per-order confirmation.
6. A 2026-09-23 host-automation probe confirmed that the local Codex CLI has the existing `binance-agent-os` configured, without re-login. Its non-interactive default approval policy is `never`. Temporarily allowing only `tool_search` succeeded, but `spot.getAccount` was not callable as a direct independent tool. The generic `tool_execute` needed for it failed with `MCP tool call requires approval, but approval policy is never`. Unconditionally approving that generic dispatcher could also forward live writes, so it was not enabled. No global policy, account, OAuth state, or trade was changed.

### 宿主接线门槛 / Host-orchestration gate

当前只允许已经验证的交互式 Codex 宿主完成 MCP 私有读取和逐笔确认写入。任何后台调度方案必须先证明：只读工具能在受限白名单下自动运行；通用执行入口不能因此获得无条件写权限；每笔写动作仍能绑定本地票据并取得用户当笔确认；失败、过期和重启均能恢复而不重复下单。证明之前不得把 `codex exec`、定时任务或观察器标为持续实盘运行器，也不得偷偷转向 API Key、Wallet 或重授权。

Until a host path proves automated read-only calls with per-order write confirmation and recovery, neither `codex exec` nor a scheduler is an accepted continuous live runner. Blanket approval of the generic MCP executor is not an acceptable shortcut.
