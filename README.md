...
| MCP执行安全契约 / MCP execution safety | 四项必需只读核对、限时账户绑定、确定性客户端订单ID、原子日志及UNKNOWN只查单恢复 / Four required read-only checks, expiring account binding, deterministic client order ID, atomic journal and lookup-only UNKNOWN recovery | 离线契约和测试可用；没有生产下单调用 / Offline contract and tests available; no production order call |
...
- 本地Python 3.11完整回归：**294项通过**。原生桌面合成验收：650轮刷新，包含38次故障注入。<br>
  Local Python 3.11 regression: **294 passed**. Native synthetic desktop acceptance:
...
- 本地自动化测试 / Local automated tests: `294 passed`
