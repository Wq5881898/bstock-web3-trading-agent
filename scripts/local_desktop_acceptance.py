...
def main():
    ...
    def check():
        ...
        try:
            ...
            window.strategy_choice.setCurrentIndex(5)
            app.processEvents()
            assert window.guarded_range_inputs["dynamic_stop_multiplier"].isVisible()
            assert window.grab().save(str(root / "range-median-guarded.png"))
            window.strategy_choice.setCurrentIndex(6)
            app.processEvents()
            assert window.range_inputs["short"].isVisible()
            assert window.grab().save(str(root / "range-ema-guarded.png"))
            window.strategy_choice.setCurrentIndex(7)
            app.processEvents()
            assert window.auto_range_inputs["ranges_bps"].isVisible()
            assert window.grab().save(str(root / "range-auto.png"))
            window.strategy_choice.setCurrentIndex(8)
            app.processEvents()
            assert window.auto_range_inputs["ranges_bps"].isVisible()
            assert window.grab().save(str(root / "range-guarded-auto.png"))
            window.strategy_choice.setCurrentIndex(9)
            app.processEvents()
            assert window.adaptive_candidates.isVisible()
            assert window.grab().save(str(root / "range-median-adaptive.png"))
